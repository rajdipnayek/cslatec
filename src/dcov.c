/* dcov.f -- translated by f2c (version 12.02.01).
...
/* Subroutine */ int dcov_(S_fp fcn, integer *iopt, integer *m, integer *n, 
	doublereal *x, doublereal *fvec, doublereal *r__, integer *ldr, 
	integer *info, doublereal *wa1, doublereal *wa2, doublereal *wa3, 
	doublereal *wa4)
{
    /* Initialized data */
    ...
    extern /* Subroutine */ int dfdjc3_(S_fp, integer *, integer *, 
	    ...
	    doublereal *, doublereal *), dqrfac_(integer *, integer *, 
...
L100:
    dfdjc3_((S_fp)fcn, m, n, &x[1], &fvec[1], &r__[r_offset], ldr, &iflag, &
	    zero, &wa4[1]);
L110:
    if (iflag < 0) {
	goto L300;
...
/*     COMPUTE THE QR DECOMPOSITION */
    dqrfac_(m, n, &r__[r_offset], ldr, &c_false, &idum, &c__1, &wa1[1], &wa1[
	    1], &wa1[1]);
    i__1 = *n;
...
/* L210: */
	...
	temp = fvec[i__];
	dwupdt_(n, &r__[r_offset], ldr, &wa1[1], &wa2[1], &temp, &wa3[1], &
		wa4[1]);
/* L220: */
    }
...
L300:
	...
	*info = 2;
    ...
    if (*info < 0) {
	xermsg_("SLATEC", "DCOV", "EXECUTION TERMINATED BECAUSE USER SET IFL"
		"AG NEGATIVE.", &c__1, &c__1, (ftnlen)6, (ftnlen)4, (ftnlen)53)
		;
    }
    if (*info == 0) {
	xermsg_("SLATEC", "DCOV", "INVALID INPUT PARAMETER.", &c__2, &c__1, (
		ftnlen)6, (ftnlen)4, (ftnlen)24);
    }
    if (*info == 2) {
	xermsg_("SLATEC", "DCOV", "SINGULAR JACOBIAN MATRIX, COVARIANCE MATR"
