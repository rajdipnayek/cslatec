/* la05as.f -- translated by f2c (version 12.02.01).
...
/* DECK LA05AS */
/* Subroutine */ int la05as_(real *a, integer *ind, integer *nz, integer *ia, 
	integer *n, integer *ip, integer *iw, real *w, real *g, real *u)
{
    /* Initialized data */
    ...
    extern /* Subroutine */ int mc20as_(integer *, integer *, real *, integer 
	    *, integer *, integer *, integer *), la05es_(real *, integer *, 
...
/* FLUSH OUT SMALL ENTRIES, COUNT ELEMENTS IN ROWS AND COLUMNS */
	...
	i__2 = la05ds_1.lenu;
	for (k = l; k <= i__2; ++k) {
	    if ((r__1 = a[k], dabs(r__1)) <= la05ds_1.small) {
		goto L70;
	    }
	    i__ = ind[k + ind_dim1];
	    j = ind[k + (ind_dim1 << 1)];
/* Computing MAX */
	    r__2 = (r__1 = a[k], dabs(r__1));
	    *g = dmax(r__2,*g);
	    if (i__ < 1 || i__ > *n) {
		goto L680;
	    }
...
L70:
	l = k;
	a[l] = a[la05ds_1.lenu];
	ind[l + ind_dim1] = ind[la05ds_1.lenu + ind_dim1];
	ind[l + (ind_dim1 << 1)] = ind[la05ds_1.lenu + (ind_dim1 << 1)];
	--la05ds_1.lenu;
/* L80: */
    }
...
L90:
    la05ds_1.lenl = 0;
    la05ds_1.lrow = la05ds_1.lenu;
    la05ds_1.lcol = la05ds_1.lrow;
...
/*     IS USED WE AUTOMATICALLY LEAVE IT POINTING TO THE FIRST ELEMENT. */
    mc20as_(n, &la05ds_1.lenu, &a[1], &ind[(ind_dim1 << 1) + 1], &ip[
...
/* Computing MAX */
			r__2 = amax, r__3 = (r__1 = a[kk], dabs(r__1));
			amax = dmax(r__2,r__3);
...
/* COMPRESS ROW FILE UNLESS IT IS CERTAIN THAT THERE IS ROOM FOR NEW ROW. */
	    if (la05ds_1.lrow + iw[ir + iw_dim1] + iw[ipp + iw_dim1] + 
		    la05ds_1.lenl <= *ia) {
		goto L350;
	    }
	    if (la05ds_1.ncp >= mcp || la05ds_1.lenu + iw[ir + iw_dim1] + iw[
		    ipp + iw_dim1] + la05ds_1.lenl > *ia) {
		goto L710;
	    }
	    la05es_(&a[1], &ind[(ind_dim1 << 1) + 1], &ip[ip_offset], n, &iw[
		    iw_offset], ia, &c_true);
	    kp = ip[ipp + ip_dim1];
	    kr = ip[ir + ip_dim1];
L350:
...
/* Computing MAX */
		...
		*g = dmax(r__1,r__2);
		++la05ds_1.lrow;
		a[la05ds_1.lrow] = au;
		ind[la05ds_1.lrow + (ind_dim1 << 1)] = j;
		goto L410;
L380:
		--la05ds_1.lenu;
...
/* IF POSSIBLE PLACE NEW ELEMENT AT END OF PRESENT ENTRY. */
		...
		if (la05ds_1.lcol + la05ds_1.lenl >= *ia) {
		    goto L460;
		}
		++la05ds_1.lcol;
		goto L450;
...
L460:
		if (la05ds_1.lcol + la05ds_1.lenl + *nz + 1 < *ia) {
		    goto L470;
		}
/* COMPRESS COLUMN FILE IF THERE IS NOT ROOM FOR NEW ENTRY. */
		if (la05ds_1.ncp >= mcp || la05ds_1.lenu + la05ds_1.lenl + *
			nz + 1 >= *ia) {
		    goto L710;
		}
		la05es_(&a[1], &ind[ind_offset], &ip[(ip_dim1 << 1) + 1], n, &
			iw[(iw_dim1 << 1) + 1], ia, &c_false);
		k = ip[j + (ip_dim1 << 1)];
		kl = k + *nz - 1;
/* TRANSFER OLD ENTRY INTO NEW. */
L470:
		ip[j + (ip_dim1 << 1)] = la05ds_1.lcol + 1;
		if (kl < k) {
		    goto L485;
		}
		i__4 = kl;
		for (kk = k; kk <= i__4; ++kk) {
		    ++la05ds_1.lcol;
		    ind[la05ds_1.lcol + ind_dim1] = ind[kk + ind_dim1];
		    ind[kk + ind_dim1] = 0;
/* L480: */
		}
L485:
...
L520:
	    iw[ir + iw_dim1] = la05ds_1.lrow + 1 - ip[ir + ip_dim1];

/* STORE MULTIPLIER */
	    if (la05ds_1.lenl + la05ds_1.lcol + 1 <= *ia) {
		goto L530;
	    }
/* COMPRESS COL FILE IF NECESSARY. */
	    ...
	    la05es_(&a[1], &ind[ind_offset], &ip[(ip_dim1 << 1) + 1], n, &iw[(
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "LA05AS", ch__1, &c_n4, &c__1, (ftnlen)6, (ftnlen)6,
		 (ftnlen)67);
    }
    *g = -4.f;
    return 0;
...
L670:
    if (la05ds_1.lp > 0) {
	xermsg_("SLATEC", "LA05AS", "THE ORDER OF THE SYSTEM, N, IS NOT POSI"
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "LA05AS", ch__2, &c_n3, &c__1, (ftnlen)6, (ftnlen)6,
		 (ftnlen)78);
    }
    *g = -3.f;
    return 0;
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "LA05AS", ch__3, &c_n2, &c__1, (ftnlen)6, (ftnlen)6,
		 (ftnlen)53);
    }
    *g = -2.f;
    return 0;
...
L710:
    if (la05ds_1.lp > 0) {
	xermsg_("SLATEC", "LA05AS", "LENGTHS OF ARRAYS A(*) AND IND(*,2) ARE"
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "LA05AS", ch__4, &c_n5, &c__1, (ftnlen)6, (ftnlen)6,
		 (ftnlen)18);
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "LA05AS", ch__5, &c_n5, &c__1, (ftnlen)6, (ftnlen)6,
		 (ftnlen)50);
	i__ += 2;
	if (i__ <= ipv) {
	    goto L740;
