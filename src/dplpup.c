/* dplpup.f -- translated by f2c (version 12.02.01).
...
/* DECK DPLPUP */
/* Subroutine */ int dplpup_(S_fp dusrmt, integer *mrelas, integer *nvars__, 
	doublereal *prgopt, doublereal *dattrv, doublereal *bl, doublereal *
	bu, integer *ind, integer *info, doublereal *amat, integer *imat, 
	logical *sizeup, doublereal *asmall, doublereal *abig)
{
    /* System generated locals */
    address a__1[3], a__2[7], a__3[5];
    integer i__1, i__2[3], i__3[7], i__4, i__5[5];
    char ch__1[57], ch__2[131], ch__3[55], ch__4[129], ch__5[74], ch__6[75];

    /* Local variables */
    static integer i__, j;
    static doublereal aij, amn, amx, xval, zero;
    static char xern1[8], xern2[8], xern3[16], xern4[16];
    static integer iflag[10], index, itcnt, itmax;
    static logical first;
    static integer iplace, indcat;
    extern /* Subroutine */ int dpchng_(integer *, doublereal *, integer *, 
	    doublereal *, integer *, integer *), xermsg_(char *, char *, char 
	    *, integer *, integer *, ftnlen, ftnlen, ftnlen), dpnnzr_(integer 
...
/* Writing concatenation */
	    i__2[0] = 33, a__1[0] = "IN DSPLP, INDEPENDENT VARIABLE = ";
	    i__2[1] = 8, a__1[1] = xern1;
	    i__2[2] = 16, a__1[2] = " IS NOT DEFINED.";
	    s_cat(ch__1, a__1, i__2, &c__3, (ftnlen)57);
	    xermsg_("SLATEC", "DPLPUP", ch__1, &c__10, &c__1, (ftnlen)6, (
		    ftnlen)6, (ftnlen)57);
	    *info = -10;
	    return 0;
	}
	...
	if (ind[j] == 3) {
		...
		e_wsfi();
/* Writing concatenation */
		i__3[0] = 24, a__2[0] = "IN DSPLP, LOWER BOUND = ";
		...
		s_cat(ch__2, a__2, i__3, &c__7, (ftnlen)131);
		xermsg_("SLATEC", "DPLPUP", ch__2, &c__11, &c__1, (ftnlen)6, (
			ftnlen)6, (ftnlen)131);
		return 0;
	    }
	}
/* L10: */
	...
	if (ind[i__] < 1 || ind[i__] > 4) {
	    ...
	    e_wsfi();
/* Writing concatenation */
	    i__2[0] = 31, a__1[0] = "IN DSPLP, DEPENDENT VARIABLE = ";
	    i__2[1] = 8, a__1[1] = xern1;
	    i__2[2] = 16, a__1[2] = " IS NOT DEFINED.";
	    s_cat(ch__3, a__1, i__2, &c__3, (ftnlen)55);
	    xermsg_("SLATEC", "DPLPUP", ch__3, &c__12, &c__1, (ftnlen)6, (
		    ftnlen)6, (ftnlen)55);
	    *info = -12;
	    return 0;
	}
	...
	if (ind[i__] == 3) {
		...
		e_wsfi();
		s_wsfi(&io___13);
		do_fio(&c__1, (char *)&bl[i__], (ftnlen)sizeof(doublereal));
		e_wsfi();
		s_wsfi(&io___14);
		do_fio(&c__1, (char *)&bu[i__], (ftnlen)sizeof(doublereal));
		e_wsfi();
/* Writing concatenation */
		i__3[0] = 24, a__2[0] = "IN DSPLP, LOWER BOUND = ";
		i__3[1] = 16, a__2[1] = xern3;
		...
		i__3[5] = 8, a__2[5] = xern1;
		i__3[6] = 20, a__2[6] = " ARE NOT CONSISTENT.";
		s_cat(ch__4, a__2, i__3, &c__7, (ftnlen)129);
		xermsg_("SLATEC", "DPLPUP", ch__4, &c__13, &c__1, (ftnlen)6, (
			ftnlen)6, (ftnlen)129);
		*info = -13;
		return 0;
	    }
...
L30:
    ++itcnt;
    if (itcnt > itmax) {
	xermsg_("SLATEC", "DPLPUP", "IN DSPLP, MORE THAN 2*NVARS*MRELAS ITER"
		"ATIONS DEFINING OR UPDATING MATRIX DATA.", &c__7, &c__1, (
		ftnlen)6, (ftnlen)6, (ftnlen)79);
	*info = -7;
	return 0;
    }

    aij = zero;
    (*dusrmt)(&i__, &j, &aij, &indcat, &prgopt[1], &dattrv[1], iflag);
    if (iflag[0] == 1) {
	iflag[0] = 2;
	goto L30;
    }
...
/*        RECORD THE LARGEST AND SMALLEST(IN MAGNITUDE) NONZERO ELEMENTS. */
	...
	e_wsfi();
/* Writing concatenation */
	i__5[0] = 22, a__3[0] = "IN DSPLP, ROW INDEX = ";
	i__5[1] = 8, a__3[1] = xern1;
	i__5[2] = 19, a__3[2] = " OR COLUMN INDEX = ";
	i__5[3] = 8, a__3[3] = xern2;
	i__5[4] = 17, a__3[4] = " IS OUT OF RANGE.";
	s_cat(ch__5, a__3, i__5, &c__5, (ftnlen)74);
	xermsg_("SLATEC", "DPLPUP", ch__5, &c__8, &c__1, (ftnlen)6, (ftnlen)6,
		 (ftnlen)74);
	*info = -8;
	return 0;
    }
...
/*     IF INDCAT=1 THEN ACCUMULATE ELEMENT, A(I,J)=A(I,J)+AIJ. */
    ...
    if (indcat == 0) {
	dpchng_(&i__, &aij, &iplace, &amat[1], &imat[1], &j);
    } else if (indcat == 1) {
	index = -(i__ - 1);
	dpnnzr_(&index, &xval, &iplace, &amat[1], &imat[1], &j);
	if (index == i__) {
	    aij += xval;
	}
	dpchng_(&i__, &aij, &iplace, &amat[1], &imat[1], &j);
	...
	e_wsfi();
/* Writing concatenation */
	i__2[0] = 28, a__1[0] = "IN DSPLP, INDICATION FLAG = ";
	i__2[1] = 8, a__1[1] = xern1;
	i__2[2] = 39, a__1[2] = " FOR MATRIX DATA MUST BE EITHER 0 OR 1.";
	s_cat(ch__6, a__1, i__2, &c__3, (ftnlen)75);
	xermsg_("SLATEC", "DPLPUP", ch__6, &c__9, &c__1, (ftnlen)6, (ftnlen)6,
		 (ftnlen)75);
	*info = -9;
	return 0;
    }
...
L40:
	...
	if (amn < *asmall || amx > *abig) {
	    xermsg_("SLATEC", "DPLPUP", "IN DSPLP, A MATRIX ELEMENT'S SIZE I"
		    "S OUT OF THE SPECIFIED RANGE.", &c__22, &c__1, (ftnlen)6, 
		    (ftnlen)6, (ftnlen)64);
	    *info = -22;
	    return 0;
	}
