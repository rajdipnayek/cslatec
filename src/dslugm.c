/* dslugm.f -- translated by f2c (version 12.02.01).
...
/* DECK DSLUGM */
/* Subroutine */ int dslugm_(integer *n, doublereal *b, doublereal *x, 
	...
	doublereal *rwork, integer *lenw, integer *iwork, integer *leniw)
{
    ...
    static integer locil, locjl, lociu, lociw, locju, locnr;
...
/*         Check the workspace allocations. */
    dchkw_("DSLUGM", &lociw, leniw, &locw, lenw, ierr, iter, err, (ftnlen)6);
    if (*ierr != 0) {
	return 0;
    }
...
/*         JPRE = -1, NRMAX = ITMAX/NSAVE */
    ...
    dgmres_(n, &b[1], &x[1], nelt, &ia[1], &ja[1], &a[1], isym, (U_fp)dsmv_, (
	    U_fp)dslui_, &myitol, tol, itmax, iter, err, ierr, iunit, &rwork[
