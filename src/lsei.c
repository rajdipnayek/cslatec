/* lsei.f -- translated by f2c (version 12.02.01).
...
static real c_b95 = 0.f;

/* DECK LSEI */
/* Subroutine */ int lsei_(real *w, integer *mdw, integer *me, integer *ma, 
	...
	rnorml, integer *mode, real *ws, integer *ip)
{
    ...
    extern /* Subroutine */ int h12_(integer *, integer *, integer *, integer 
	    *, real *, integer *, real *, real *, integer *, integer *, 
	    integer *);
    static real rb, uj, rn, sn, vj, up;
    static integer jp1, np1;
    static real gam;
    extern /* Subroutine */ int lsi_(real *, integer *, integer *, integer *, 
	    integer *, real *, real *, real *, integer *, real *, integer *);
    static real tau;
    static integer key;
    static logical cov;
    static integer mep1, lchk, mend, link, imax, last;
...
/* Writing concatenation */
	    ...
	    xermsg_("SLATEC", "LSEI", ch__2, &c__2, &c__1, (ftnlen)6, (ftnlen)
		    4, (ftnlen)60);
	    return 0;
	}
    }
...
/* Writing concatenation */
	    ...
	    xermsg_("SLATEC", "LSEI", ch__3, &c__2, &c__1, (ftnlen)6, (ftnlen)
		    4, (ftnlen)61);
	    return 0;
	}
    }
...
/*     transformations. */
	...
	*rnorml = 0.f;
    ...
    if (*mdw < m) {
	xermsg_("SLATEC", "LSEI", "MDW.LT.ME+MA+MG IS AN ERROR", &c__2, &c__1,
...
/*     the identity scaling. */
    ...
    scopy_(n, &c_b41, &c__0, &ws[n1], &c__1);
...
/*     Define bound for positive values of LINK. */
	...
	xermsg_("SLATEC", "LSEI", "THE OPTION VECTOR IS UNDEFINED", &c__2, &
...
L100:
	...
	++ntimes;
	if (ntimes > nopt) {
	    xermsg_("SLATEC", "LSEI", "THE LINKS IN THE OPTION VECTOR ARE CY"
		    "CLING.", &c__2, &c__1, (ftnlen)6, (ftnlen)4, (ftnlen)43);
	    return 0;
	}
	...
	} else if (key == 2 && prgopt[last + 2] != 0.f) {
	    i__1 = *n;
	    for (j = 1; j <= i__1; ++j) {
...
/* Computing MAX */
    ...
    i__1 = *n;
    for (j = 1; j <= i__1; ++j) {
	sscal_(&m, &ws[n1 + j - 1], &w[j * w_dim1 + 1], &c__1);
/* L120: */
    }

    if (cov && *mdw < *n) {
	xermsg_("SLATEC", "LSEI", "MDW .LT. N WHEN COV MATRIX NEEDED, IS AN "
		"ERROR", &c__2, &c__1, (ftnlen)6, (ftnlen)4, (ftnlen)46);
	return 0;
    }
...
/*        column I. */
	...
	for (k = i__; k <= i__4; ++k) {
	    i__5 = *n - i__ + 1;
	    sn = sdot_(&i__5, &w[k + i__ * w_dim1], mdw, &w[k + i__ * w_dim1],
		     mdw);
	    i__5 = i__ - 1;
	    rn = sdot_(&i__5, &w[k + w_dim1], mdw, &w[k + w_dim1], mdw);
	    if (rn == 0.f && sn > snmax) {
		snmax = sn;
		imax = k;
...
/* Computing 2nd power */
	r__1 = tau;
	if (snmax > rnmax * (r__1 * r__1)) {

/*        Eliminate elements I+1,...,N in row I. */

	    i__4 = i__ + 1;
	    i__5 = m - i__;
	    h12_(&c__1, &i__, &i__4, n, &w[i__ + w_dim1], mdw, &ws[i__], &w[
...
/*           Apply transformation to matrix cols. 1,...,K-1. */
	    ...
	    i__1 = kranke + 1;
	    i__4 = k - 1;
	    h12_(&c__1, &k, &i__1, me, &w[k * w_dim1 + 1], &c__1, &up, &w[
		    w_offset], &c__1, mdw, &i__4);

/*           Apply to rt side vector. */

	    i__1 = kranke + 1;
	    h12_(&c__2, &k, &i__1, me, &w[k * w_dim1 + 1], &c__1, &up, &w[np1 
...
/*     Solve for variables 1,...,KRANKE in new coordinates. */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
	i__4 = i__ - 1;
	x[i__] = (x[i__] - sdot_(&i__4, &w[i__ + w_dim1], mdw, &x[1], &c__1)) 
		/ w[i__ + i__ * w_dim1];
/* L180: */
    }
...
/*     Compute residuals for reduced problem. */
    ...
    *rnorml = 0.f;
	...
	i__4 = *n - kranke;
	rn = sdot_(&i__4, &w[i__ + (kranke + 1) * w_dim1], mdw, &w[i__ + (
		kranke + 1) * w_dim1], mdw);
/* Computing 2nd power */
	r__1 = tau;
	if (rn <= sn * (r__1 * r__1) && kranke < *n) {
	    i__4 = *n - kranke;
	    scopy_(&i__4, &c_b95, &c__0, &w[i__ + (kranke + 1) * w_dim1], mdw)
		    ;
	}
/* L190: */
    }
...
/*     Move reduced problem data upward if KRANKE.LT.ME. */
	...
	for (j = 1; j <= i__1; ++j) {
	    i__4 = m - *me;
	    scopy_(&i__4, &w[*me + 1 + j * w_dim1], &c__1, &w[kranke + 1 + j *
		     w_dim1], &c__1);
/* L200: */
	}
    }
...
/*        constraints when there are no degrees of freedom left. */
	...
	if (kranke == *n && *mg > 0) {
		...
		size = sasum_(n, &w[i__ + w_dim1], mdw) * xnorm + (r__1 = w[
			i__ + np1 * w_dim1], dabs(r__1));
...
/*        Reapply transformation to put solution in original coordinates. */
	...
	for (i__ = kranke; i__ >= 1; --i__) {
	    i__1 = i__ + 1;
	    h12_(&c__2, &i__, &i__1, n, &w[i__ + w_dim1], mdw, &ws[i__], &x[1]
		    , &c__1, &c__1, &c__1);
/* L220: */
	}
...
/* Computing MIN */
		...
		for (i__ = jp1; i__ <= i__1; ++i__) {
		    i__4 = *n - j;
		    w[i__ + j * w_dim1] = rb * sdot_(&i__4, &w[i__ + jp1 * 
			    w_dim1], mdw, &w[j + jp1 * w_dim1], mdw);
/* L230: */
		}

		i__1 = *n - j;
		gam = rb * .5f * sdot_(&i__1, &w[jp1 + j * w_dim1], &c__1, &w[
			j + jp1 * w_dim1], mdw);
		i__1 = *n - j;
		saxpy_(&i__1, &gam, &w[j + jp1 * w_dim1], mdw, &w[jp1 + j * 
			w_dim1], &c__1);
		i__1 = *n;
		for (i__ = jp1; i__ <= i__1; ++i__) {
		    i__4 = *n;
...
/* L260: */
		...
		i__1 = *n - j;
		scopy_(&i__1, &w[j + jp1 * w_dim1], mdw, &w[jp1 + j * w_dim1],
			 &c__1);
/* L270: */
	    }
	}
...
/*     Apply the scaling to the covariance matrix. */
	...
	i__1 = *n;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    sscal_(n, &ws[i__ + n1 - 1], &w[i__ + w_dim1], mdw);
	    sscal_(n, &ws[i__ + n1 - 1], &w[i__ * w_dim1 + 1], &c__1);
/* L280: */
	}
    }
