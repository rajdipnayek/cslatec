/* dbsk1e.f -- translated by f2c (version 12.02.01).
...
doublereal dbsk1e_(doublereal *x)
{
    ...
    static doublereal ak1cs[38] = { .27443134069738829695257666227266,
    ...
    static doublereal ak12cs[33] = { .06379308343739001036600488534102,
    ...
    static real eta;
    static integer ntk1;
    static doublereal xmin, xsml;
    static integer ntak1, ntak12;
    extern doublereal d1mach_(integer *), dbesi1_(doublereal *), dcsevl_(
...
/* ***FIRST EXECUTABLE STATEMENT  DBSK1E */
	...
	ntk1 = initds_(bk1cs, &c__16, &eta);
	ntak1 = initds_(ak1cs, &c__38, &eta);
	ntak12 = initds_(ak12cs, &c__33, &eta);
...
/* Computing MAX */
    ...
    d__1 = y * .5 - 1.;
    ret_val = exp(*x) * (log(*x * .5) * dbesi1_(x) + (dcsevl_(&d__1, bk1cs, &
	    ntk1) + .75) / *x);
    return ret_val;
...
L20:
    if (*x <= 8.) {
	d__1 = (16. / *x - 5.) / 3.;
	ret_val = (dcsevl_(&d__1, ak1cs, &ntak1) + 1.25) / sqrt(*x);
    }
    if (*x > 8.) {
	d__1 = 16. / *x - 1.;
	ret_val = (dcsevl_(&d__1, ak12cs, &ntak12) + 1.25) / sqrt(*x);
    }
