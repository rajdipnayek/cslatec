/* ohtrol.f -- translated by f2c (version 12.02.01).
...
/* DECK OHTROL */
/* Subroutine */ int ohtrol_(real *q, integer *n, integer *nrda, real *diag, 
...
/* ***FIRST EXECUTABLE STATEMENT  OHTROL */
	...
	kir = irp - k;
	diagk = diag[kir];
	sig = diagk * diagk + sdot_(&nmir, &q[irp + kir * q_dim1], &c__1, &q[
		irp + kir * q_dim1], &c__1);
	r__1 = sqrt(sig);
	r__2 = -diagk;
	dd = r_sign(&r__1, &r__2);
	...
	i__2 = kirm;
	for (j = 1; j <= i__2; ++j) {
	    qs = (tdv * q[kir + j * q_dim1] + sdot_(&nmir, &q[irp + j * 
