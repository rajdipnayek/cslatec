/* pchia.f -- translated by f2c (version 12.02.01).
...
/* DECK PCHIA */
doublereal pchia_(integer *n, real *x, real *f, real *d__, integer *incfd, 
	logical *skip, real *a, real *b, integer *ierr)
{
    /* Initialized data */
    ...
    extern doublereal chfie_(real *, real *, real *, real *, real *, real *, 
	    real *, real *), pchid_(integer *, real *, real *, real *, 
...
/*                   -------------------------------------- */
	    value = chfie_(&x[1], &x[2], &f[f_dim1 + 1], &f[(f_dim1 << 1) + 1]
		    , &d__[d_dim1 + 1], &d__[(d_dim1 << 1) + 1], a, b);
/*                   -------------------------------------- */
	} else if (xa >= x[*n - 1]) {
/*           INTERVAL IS TO RIGHT OF X(N-1), SO USE LAST CUBIC. */
/*                   ----------------------------------------- */
	    value = chfie_(&x[*n - 1], &x[*n], &f[(*n - 1) * f_dim1 + 1], &f[*
...
/*                      ------------------------------------------ */
		value = chfie_(&x[ib], &x[ia], &f[ib * f_dim1 + 1], &f[ia * 
...
/*                 of VALUE to ZERO.) */
		if (ib > ia) {
/*                         --------------------------------------------- */
		    value = pchid_(n, &x[1], &f[f_offset], &d__[d_offset], 
...
/* Computing MAX */
		    ...
		    ir = il + 1;
/*                                 ------------------------------------- */
		    value += chfie_(&x[il], &x[ir], &f[il * f_dim1 + 1], &f[
...
/*     N.LT.2 RETURN. */
    *ierr = -1;
    xermsg_("SLATEC", "PCHIA", "NUMBER OF DATA POINTS LESS THAN TWO", ierr, &
...
/*     INCFD.LT.1 RETURN. */
    *ierr = -2;
    xermsg_("SLATEC", "PCHIA", "INCREMENT LESS THAN ONE", ierr, &c__1, (
...
/*     X-ARRAY NOT STRICTLY INCREASING. */
    *ierr = -3;
    xermsg_("SLATEC", "PCHIA", "X-ARRAY NOT STRICTLY INCREASING", ierr, &c__1,
...
/*     TROUBLE IN PCHID.  (SHOULD NEVER OCCUR.) */
    *ierr = -4;
    xermsg_("SLATEC", "PCHIA", "TROUBLE IN PCHID", ierr, &c__1, (ftnlen)6, (
	    ftnlen)5, (ftnlen)16);
    goto L5000;
