/* ddcst.f -- translated by f2c (version 12.02.01).
...
/* DECK DDCST */
/* Subroutine */ int ddcst_(integer *maxord, integer *mint, integer *iswflg, 
	doublereal *el, doublereal *tq)
{
    ...
    static integer mxrd;
    static doublereal gamma[14], factrl[12];
...
/* L80: */
	...
	i__2 = *maxord;
	for (j = 1; j <= i__2; ++j) {
	    tq[j * 3 + 1] = -1. / (factrl[j - 1] * gamma[j - 1]);
	    tq[j * 3 + 2] = -1. / gamma[j];
/* L100: */
	    tq[j * 3 + 3] = -1. / gamma[j + 1];
	}
