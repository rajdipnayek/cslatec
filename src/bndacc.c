/* bndacc.f -- translated by f2c (version 12.02.01).
...
/* DECK BNDACC */
/* Subroutine */ int bndacc_(real *g, integer *mdg, integer *nb, integer *ip, 
	integer *ir, integer *mt, integer *jt)
{
    ...
    extern /* Subroutine */ int h12_(integer *, integer *, integer *, integer 
	    *, real *, integer *, real *, real *, integer *, integer *, 
	    integer *);
    static integer ie, ig, jg, kh, mh, mu, ig1, ig2, lp1;
    static real rho;
...
/*              ALG. STEPS 1-4 ARE PERFORMED EXTERNAL TO THIS SUBROUTINE. */
    ...
    nerr = 1;
    iopt = 2;
    xermsg_("SLATEC", "BNDACC", "MDG.LT.IR, PROBABLE ERROR.", &nerr, &iopt, (
...
/* Computing MAX */
	...
	i__1 = max(i__3,i__4);
	i__5 = nbp1 - i__;
	h12_(&c__1, &i__, &i__1, &mh, &g[*ip + i__ * g_dim1], &c__1, &rho, &g[
		*ip + (i__ + 1) * g_dim1], &c__1, mdg, &i__5);
/* L80: */
    }
/*                                             ALG. STEP 21 */
    *ir = *ip + kh;
