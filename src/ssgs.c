/* ssgs.f -- translated by f2c (version 12.02.01).
	...
	leniw)
{
    ...
    extern /* Subroutine */ int ssli_(), ssmv_();
    extern /* Subroutine */ int ss2lt_(integer *, integer *, integer *, 
...
/*         Check the workspace allocations. */
    ...
    ss2lt_(n, nelt, &ia[1], &ja[1], &a[1], isym, &nl, &iwork[lociel], &iwork[
	    locjel], &rwork[locel]);

/*         Call iterative refinement routine. */
    sir_(n, &b[1], &x[1], nelt, &ia[1], &ja[1], &a[1], isym, (U_fp)ssmv_, (
	    U_fp)ssli_, itol, tol, itmax, iter, err, ierr, iunit, &rwork[locr]
