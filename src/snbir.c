/* snbir.f -- translated by f2c (version 12.02.01).
...
/* DECK SNBIR */
/* Subroutine */ int snbir_(real *abe, integer *lda, integer *n, integer *ml, 
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "SNBIR", ch__1, &c_n1, &c__1, (ftnlen)6, (ftnlen)5, 
		(ftnlen)40);
	return 0;
    }
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "SNBIR", ch__2, &c_n2, &c__1, (ftnlen)6, (ftnlen)5, 
		(ftnlen)27);
	return 0;
    }
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "SNBIR", ch__3, &c_n3, &c__1, (ftnlen)6, (ftnlen)5, 
		(ftnlen)31);
	return 0;
    }
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "SNBIR", ch__4, &c_n5, &c__1, (ftnlen)6, (ftnlen)5, 
		(ftnlen)29);
	return 0;
    }
...
/* Writing concatenation */
	...
	i__2[2] = 16, a__2[2] = " IS OUT OF RANGE";
	s_cat(ch__4, a__2, i__2, &c__3, (ftnlen)29);
	xermsg_("SLATEC", "SNBIR", ch__4, &c_n6, &c__1, (ftnlen)6, (ftnlen)5, 
		(ftnlen)29);
	return 0;
    }
...
/*        MOVE MATRIX ABE TO WORK */
	...
	i__3 = m;
	for (j = 1; j <= i__3; ++j) {
	    scopy_(n, &abe[j * abe_dim1 + 1], &c__1, &work[j * work_dim1 + 1],
		     &c__1);
/* L10: */
	}
...
/*        FACTOR MATRIX A INTO LU */
	...
	snbfa_(&work[work_offset], n, n, ml, mu, &iwork[1], &info);

/*        CHECK FOR COMPUTATIONALLY SINGULAR MATRIX */

	if (info != 0) {
	    *ind = -4;
	    xermsg_("SLATEC", "SNBIR", "SINGULAR MATRIX A - NO SOLUTION", &
...
/*     MOVE VECTOR B TO WORK */
    ...
    scopy_(n, &v[1], &c__1, &work[(nc + 1) * work_dim1 + 1], &c__1);
    snbsl_(&work[work_offset], n, n, ml, mu, &iwork[1], &v[1], &c__0);
...
/* Computing MIN */
	...
	r__1 = -work[j + (nc + 1) * work_dim1];
	work[j + (nc + 1) * work_dim1] = sdsdot_(&l, &r__1, &abe[j + k * 
		abe_dim1], lda, &v[kk], &c__1);
/* L40: */
    }

/*     SOLVE A*DELTA=R */

    snbsl_(&work[work_offset], n, n, ml, mu, &iwork[1], &work[(nc + 1) * 
	    work_dim1 + 1], &c__0);

/*     FORM NORM OF DELTA */

    dnorm = sasum_(n, &work[(nc + 1) * work_dim1 + 1], &c__1);
...
/* Computing MAX */
    ...
    *ind = -r_lg10(&r__1);
    if (*ind <= 0) {
	*ind = -10;
	xermsg_("SLATEC", "SNBIR", "SOLUTION MAY HAVE NO SIGNIFICANCE", &
