/* pchic.f -- translated by f2c (version 12.02.01).
...
/* DECK PCHIC */
/* Subroutine */ int pchic_(integer *ic, real *vc, real *switch__, integer *n,
	 real *x, real *f, real *d__, integer *incfd, real *wk, integer *nwk, 
	integer *ierr)
{
    /* Initialized data */
    ...
    /* Local variables */
    static integer i__, ibeg, iend;
    extern /* Subroutine */ int pchce_(integer *, real *, integer *, real *, 
	    real *, real *, real *, integer *, integer *), pchci_(integer *, 
	    real *, real *, real *, integer *), pchcs_(real *, integer *, 
...
/*     -------------------------------------- */
    pchci_(n, &wk[1], &wk[*n], &d__[d_offset], incfd);
/*     -------------------------------------- */
...
/*     ---------------------------------------------------- */
    pchcs_(switch__, n, &wk[1], &wk[*n], &d__[d_offset], incfd, ierr);
/*     ---------------------------------------------------- */
    if (*ierr != 0) {
	goto L5008;
    }
...
/*     ------------------------------------------------------- */
    pchce_(&ic[1], &vc[1], n, &x[1], &wk[1], &wk[*n], &d__[d_offset], incfd, 
	    ierr);
/*     ------------------------------------------------------- */
    if (*ierr < 0) {
	goto L5009;
...
/*     N.LT.2 RETURN. */
    *ierr = -1;
    xermsg_("SLATEC", "PCHIC", "NUMBER OF DATA POINTS LESS THAN TWO", ierr, &
...
/*     INCFD.LT.1 RETURN. */
    *ierr = -2;
    xermsg_("SLATEC", "PCHIC", "INCREMENT LESS THAN ONE", ierr, &c__1, (
...
/*     X-ARRAY NOT STRICTLY INCREASING. */
    *ierr = -3;
    xermsg_("SLATEC", "PCHIC", "X-ARRAY NOT STRICTLY INCREASING", ierr, &c__1,
...
/*     IC OUT OF RANGE RETURN. */
    *ierr += -3;
    xermsg_("SLATEC", "PCHIC", "IC OUT OF RANGE", ierr, &c__1, (ftnlen)6, (
	    ftnlen)5, (ftnlen)15);
    return 0;
...
/*     NWK .LT. 2*(N-1)  RETURN. */
    *ierr = -7;
    xermsg_("SLATEC", "PCHIC", "WORK ARRAY TOO SMALL", ierr, &c__1, (ftnlen)6,
	     (ftnlen)5, (ftnlen)20);
    return 0;
...
/*     ERROR RETURN FROM PCHCS. */
    *ierr = -8;
    xermsg_("SLATEC", "PCHIC", "ERROR RETURN FROM PCHCS", ierr, &c__1, (
	    ftnlen)6, (ftnlen)5, (ftnlen)23);
    return 0;
...
/*   *** THIS CASE SHOULD NEVER OCCUR *** */
    *ierr = -9;
    xermsg_("SLATEC", "PCHIC", "ERROR RETURN FROM PCHCE", ierr, &c__1, (
	    ftnlen)6, (ftnlen)5, (ftnlen)23);
    return 0;
