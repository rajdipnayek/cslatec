/* prwpge.f -- translated by f2c (version 12.02.01).
...
/* DECK PRWPGE */
/* Subroutine */ int prwpge_(integer *key, integer *ipage, integer *lpg, real 
	*sx, integer *ix)
{
    extern /* Subroutine */ int xermsg_(char *, char *, char *, integer *, 
	    integer *, ftnlen, ftnlen, ftnlen), prwvir_(integer *, integer *, 
...
/*     CHECK IF IPAGE IS IN RANGE. */
    ...
    /* Function Body */
    if (*ipage < 1) {
	xermsg_("SLATEC", "PRWPGE", "THE VALUE OF IPAGE (PAGE NUMBER) WAS NO"
...
/*     CHECK IF LPG IS POSITIVE. */
    ...
    if (*lpg <= 0) {
	xermsg_("SLATEC", "PRWPGE", "THE VALUE OF LPG (PAGE LENGTH) WAS NONP"
...
/*        CODE TO DO A PAGE READ. */
	...
	prwvir_(key, ipage, lpg, &sx[1], &ix[1]);
    } else if (*key == 2) {
...
/*        CODE TO DO A PAGE WRITE. */
	...
	xermsg_("SLATEC", "PRWPGE", "THE VALUE OF KEY (READ-WRITE FLAG) WAS "
