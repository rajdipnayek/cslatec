/* ddriv2.f -- translated by f2c (version 12.02.01).
...
/* DECK DDRIV2 */
/* Subroutine */ int ddriv2_(integer *n, doublereal *t, doublereal *y, U_fp f,
	 doublereal *tout, integer *mstate, integer *nroot, doublereal *eps, 
	doublereal *ewt, integer *mint, doublereal *work, integer *lenw, 
	integer *iwork, integer *leniw, D_fp g, integer *ierflg)
{
...
/* ***FIRST EXECUTABLE STATEMENT  DDRIV2 */
    ...
    if (abs(*mstate) == 9) {
	*ierflg = 999;
	xermsg_("SLATEC", "DDRIV2", "Illegal input.  The magnitude of MSTATE"
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "DDRIV2", ch__1, ierflg, &c__1, (ftnlen)6, (ftnlen)
		6, (ftnlen)78);
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "DDRIV2", ch__2, ierflg, &c__1, (ftnlen)6, (ftnlen)
		6, (ftnlen)74);
	*mstate = i_sign(&c__9, mstate);
	return 0;
    }
    ...
    ddriv3_(n, t, &y[1], (U_fp)f, &nstate, tout, &ntask, nroot, eps, ewtcom, &
	    ierror, mint, &miter, &c__0, &ml, &mu, &mxord, &hmax, &work[1], 
	    lenw, &iwork[1], leniw, (U_fp)f, (U_fp)f, &nde, &c__1000, (D_fp)g,
	     (U_fp)f, ierflg);
    if (nstate <= 7) {
