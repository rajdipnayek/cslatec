/* dnls1e.f -- translated by f2c (version 12.02.01).
...
/* DECK DNLS1E */
/* Subroutine */ int dnls1e_(U_fp fcn, integer *iopt, integer *m, integer *n, 
	doublereal *x, doublereal *fvec, doublereal *tol, integer *nprint, 
	integer *info, integer *iw, doublereal *wa, integer *lwa)
{
    /* Initialized data */
    ...
    extern /* Subroutine */ int dnls1_(U_fp, integer *, integer *, integer *, 
...
/*     CALL DNLS1. */
    ...
    mode = 1;
    index = *n * 5 + *m;
    dnls1_((U_fp)fcn, iopt, m, n, &x[1], &fvec[1], &wa[index + 1], m, &ftol, &
...
L10:
    if (*info == 0) {
	xermsg_("SLATEC", "DNLS1E", "INVALID INPUT PARAMETER.", &c__2, &c__1, 
