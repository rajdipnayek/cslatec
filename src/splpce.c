/* splpce.f -- translated by f2c (version 12.02.01).
...
/* DECK SPLPCE */
/* Subroutine */ int splpce_(integer *mrelas, integer *nvars__, integer *lmx, 
	integer *lbm, integer *itlp, integer *itbrc, integer *ibasis, integer 
	*imat, integer *ibrc, integer *ipr, integer *iwr, integer *ind, 
	integer *ibb, real *erdnrm, real *eps, real *tune, real *gg, real *
	amat, real *basmat, real *csc, real *wr, real *ww, real *primal, real 
	*erd, real *erp, logical *singlr, logical *redbas)
{
...
/* Computing MAX */
    r__2 = (r__1 = ww[i__] - one, dabs(r__1));
    erd[i__] = dmax(r__2,*eps) * *tune;
...
/*     THIS VALUE (FACTOR) MIGHT NEED TO BE CHANGED. */
    *singlr = *singlr || erd[i__] >= factor;
    goto L20016;
L20018:
    *erdnrm = sasum_(mrelas, &erd[1], &c__1);
