/* sepeli.f -- translated by f2c (version 12.02.01).
...
/* DECK SEPELI */
/* Subroutine */ int sepeli_(integer *intl, integer *iorder, real *a, real *b,
	 integer *m, integer *mbdcnd, real *bda, real *alpha, real *bdb, real 
	*beta, real *c__, real *d__, integer *n, integer *nbdcnd, real *bdc, 
	real *gama, real *bdd, real *xnu, U_fp cofx, U_fp cofy, real *grhs, 
	real *usol, integer *idmn, real *w, real *pertrb, integer *ierror)
{
    /* System generated locals */
    integer grhs_dim1, grhs_offset, usol_dim1, usol_offset, i__1, i__2;

    /* Local variables */
    static integer k, l, i1, i2, i3, i4, i5, i6, i7, i8, i9, i10, i11, i12, 
	    i13, ll, logb2n, length;
    extern /* Subroutine */ int chkprm_(integer *, integer *, real *, real *, 
	    integer *, integer *, real *, real *, integer *, integer *, U_fp, 
	    U_fp, integer *, integer *), spelip_(integer *, integer *, real *,
	     real *, integer *, integer *, real *, real *, real *, real *, 
	    real *, real *, integer *, integer *, real *, real *, real *, 
	    real *, U_fp, U_fp, real *, real *, real *, real *, real *, real *
	    , real *, real *, real *, real *, real *, real *, real *, real *, 
	    integer *, real *, real *, integer *);
    static integer linput, loutpt;
...
/* ***FIRST EXECUTABLE STATEMENT  SEPELI */
    ...
    /* Function Body */
    chkprm_(intl, iorder, a, b, m, mbdcnd, c__, d__, n, nbdcnd, (U_fp)cofx, (
	    U_fp)cofy, idmn, ierror);
    if (*ierror != 0) {
	return 0;
    }
...
/*     COMPUTE MINIMUM WORK SPACE AND CHECK WORK SPACE LENGTH INPUT */
    ...
    logb2n = (integer) (log(l + .5f) / log(2.f)) + 1;
    i__1 = logb2n + 1;
    ll = pow_ii(&c__2, &i__1);
    k = *m + 1;
    l = *n + 1;
/* Computing MAX */
    i__1 = l << 1, i__2 = k * 6;
    length = (logb2n - 2) * ll + logb2n + max(i__1,i__2) + 5;
    if (*nbdcnd == 0) {
	length += l << 1;
    }
    *ierror = 11;
    linput = (integer) (w[1] + .5f);
    loutpt = length + (k + l) * 6 + 1;
    w[1] = (real) loutpt;
    if (loutpt > linput) {
	return 0;
    }
...
/*     SET WORK SPACE INDICES */
    ...
    i12 = i11 + k;
    i13 = 2;
    spelip_(intl, iorder, a, b, m, mbdcnd, &bda[1], alpha, &bdb[1], beta, c__,
	     d__, n, nbdcnd, &bdc[1], gama, &bdd[1], xnu, (U_fp)cofx, (U_fp)
	    cofy, &w[i1], &w[i2], &w[i3], &w[i4], &w[i5], &w[i6], &w[i7], &w[
	    i8], &w[i9], &w[i10], &w[i11], &w[i12], &grhs[grhs_offset], &usol[
	    usol_offset], idmn, &w[i13], pertrb, ierror);
    return 0;
} /* sepeli_ */
