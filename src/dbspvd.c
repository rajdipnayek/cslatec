/* dbspvd.f -- translated by f2c (version 12.02.01).
...
/* DECK DBSPVD */
/* Subroutine */ int dbspvd_(doublereal *t, integer *k, integer *nderiv, 
	doublereal *x, integer *ileft, integer *ldvnik, doublereal *vnikx, 
	doublereal *work)
{
    ...
    static doublereal fkmd;
    static integer jlow, mhigh, ipkmd, iwork, jp1mid;
...
/* ***FIRST EXECUTABLE STATEMENT  DBSPVD */
    ...
    kp1 = *k + 1;
    jj = kp1 - ideriv;
    dbspvn_(&t[1], &jj, k, &c__1, x, ileft, &vnikx[vnikx_offset], &work[1], &
	    iwork);
    if (ideriv == 1) {
	goto L100;
    }
...
/* L10: */
	...
	--ideriv;
	jj = kp1 - ideriv;
	dbspvn_(&t[1], &jj, k, &c__2, x, ileft, &vnikx[vnikx_offset], &work[1]
		, &iwork);
/* L20: */
    }
...
L200:
    xermsg_("SLATEC", "DBSPVD", "K DOES NOT SATISFY K.GE.1", &c__2, &c__1, (
	    ftnlen)6, (ftnlen)6, (ftnlen)25);
    return 0;
L205:
    xermsg_("SLATEC", "DBSPVD", "NDERIV DOES NOT SATISFY 1.LE.NDERIV.LE.K", &
	    c__2, &c__1, (ftnlen)6, (ftnlen)6, (ftnlen)40);
    return 0;
L210:
    xermsg_("SLATEC", "DBSPVD", "LDVNIK DOES NOT SATISFY LDVNIK.GE.K", &c__2, 
