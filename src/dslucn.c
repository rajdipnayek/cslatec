/* dslucn.f -- translated by f2c (version 12.02.01).
...
/* DECK DSLUCN */
/* Subroutine */ int dslucn_(integer *n, doublereal *b, doublereal *x, 
...
/*         Check the workspace allocations. */
    dchkw_("DSLUCN", &lociw, leniw, &locw, lenw, ierr, iter, err, (ftnlen)6);
    if (*ierr != 0) {
	return 0;
    }
...
/*         Perform Conjugate Gradient algorithm on the normal equations. */
    dcgn_(n, &b[1], &x[1], nelt, &ia[1], &ja[1], &a[1], isym, (U_fp)dsmv_, (
	    U_fp)dsmtv_, (U_fp)dsmmti_, itol, tol, itmax, iter, err, ierr, 
