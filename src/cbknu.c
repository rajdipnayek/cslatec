/* cbknu.f -- translated by f2c (version 12.02.01).
	...
	n, complex *y, integer *nz, real *tol, real *elim, real *alim)
{
    ...
    static real fpi = 1.89769999331517738f;
    static real tth = .666666666666666666f;
    static real cc[8] = { .577215664901532861f,-.0420026350340952355f,
	    -.0421977345555443367f,.00721894324666309954f,
	    -2.15241674114950973e-4f,-2.01348547807882387e-5f,
	    1.13302723198169588e-6f,6.11609510448141582e-9f };
    static real r1 = 2.f;
    static complex czero = {0.f,0.f};
    static complex cone = {1.f,0.f};
    static complex ctwo = {2.f,0.f};
    static real pi = 3.14159265358979324f;
    static real rthpi = 1.25331413731550025f;
    static real spi = 1.90985931710274403f;
...
/* ----------------------------------------------------------------------- */
    t1 = (i1mach_(&c__11) - 1) * r1mach_(&c__5) * 3.321928094f;
    t1 = dmax(t1,12.f);
    t1 = dmin(t1,60.f);
    t2 = tth * t1 - 6.f;
    if (xx != 0.f) {
...
/* ----------------------------------------------------------------------- */
    ...
    aa = t1 * 3.f / (caz + 1.f);
    bb = t1 * 14.7f / (caz + 28.f);
    ak = (log(ak) + caz * cos(aa) / (caz * .008f + 1.f)) / cos(bb);
    fk = ak * .12125f * ak / caz + 1.5f;
L180:
    k = fk;
/* ----------------------------------------------------------------------- */
