/* dusrmt.f -- translated by f2c (version 12.02.01).
...
/* DECK DUSRMT */
/* Subroutine */ int dusrmt_(integer *i__, integer *j, doublereal *aij, 
...
/*     DATTRV(*).  ALSO CHECK (DATTRV(1)=0.) SIGNIFYING NO DATA. */
	if (dattrv[1] == 0.) {
	    ...
	    *j = 0;
	    ...
	    iflag[2] = (integer) (-dattrv[1]);
	    iflag[3] = (integer) dattrv[2];
	    iflag[4] = 3;
	}
