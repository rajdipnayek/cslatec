/* dsdbcg.f -- translated by f2c (version 12.02.01).
...
/* DECK DSDBCG */
/* Subroutine */ int dsdbcg_(integer *n, doublereal *b, doublereal *x, 
	...
	integer *lenw, integer *iwork, integer *leniw)
{
    ...
    static integer locdz, lociw, locpp, locrr;
...
/*         Check the workspace allocations. */
    dchkw_("DSDBCG", &lociw, leniw, &locw, lenw, ierr, iter, err, (ftnlen)6);
    if (*ierr != 0) {
	return 0;
    }
...
/*         Perform the Diagonally Scaled BiConjugate gradient algorithm. */
    dbcg_(n, &b[1], &x[1], nelt, &ia[1], &ja[1], &a[1], isym, (U_fp)dsmv_, (
	    U_fp)dsmtv_, (U_fp)dsdi_, (U_fp)dsdi_, itol, tol, itmax, iter, 
