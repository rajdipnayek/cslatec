/* snbsl.f -- translated by f2c (version 12.02.01).
...
/* DECK SNBSL */
/* Subroutine */ int snbsl_(real *abe, integer *lda, integer *n, integer *ml, 
...
L10:
	mlm = *ml - (lm - 1);
	saxpy_(&lm, &t, &abe[k + lm + mlm * abe_dim1], &ldb, &b[k + 1], &c__1)
		;
/* L20: */
    }
L30:
...
/*       NOW SOLVE  U*X = Y */
	...
	lb = k - lm;
	t = -b[k];
	saxpy_(&lm, &t, &abe[k - 1 + (*ml + 2) * abe_dim1], &ldb, &b[lb], &
		c__1);
/* L40: */
    }
    goto L100;
...
/*       FIRST SOLVE  TRANS(U)*Y = B */
	...
	lm = min(k,m) - 1;
	lb = k - lm;
	t = sdot_(&lm, &abe[k - 1 + (*ml + 2) * abe_dim1], &ldb, &b[lb], &
		c__1);
...
/* Computing MIN */
	...
	lm = min(i__2,i__3);
	mlm = *ml - (lm - 1);
	b[k] += sdot_(&lm, &abe[k + lm + mlm * abe_dim1], &ldb, &b[k + 1], &
		c__1);
	l = ipvt[k];
	if (l == k) {
	    goto L70;
