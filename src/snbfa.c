/* snbfa.f -- translated by f2c (version 12.02.01).
...
/* DECK SNBFA */
/* Subroutine */ int snbfa_(real *abe, integer *lda, integer *n, integer *ml, 
...
/*     SEARCH FOR PIVOT INDEX */
	...
	l = -isamax_(&lm1, &abe[lm + k + lm2 * abe_dim1], &ldb) + lm1 + k;
	ipvt[k] = l;
/* Computing MIN */
	i__2 = mb, i__3 = *n - k;
	mp = min(i__2,i__3);
...
/*     SWAP ROWS IF NECESSARY */
	...
	if (l != k) {
	    i__2 = mp + 1;
	    sswap_(&i__2, &abe[k + ml1 * abe_dim1], lda, &abe[l + (ml1 + k - 
		    l) * abe_dim1], lda);
	}
...
/*     COMPUTE MULTIPLIERS */
	...
	sscal_(&lm, &t, &abe[lm + k + lm2 * abe_dim1], &ldb);

/*     ROW ELIMINATION WITH COLUMN INDEXING */

	i__2 = mp;
	for (j = 1; j <= i__2; ++j) {
	    saxpy_(&lm, &abe[k + (ml1 + j) * abe_dim1], &abe[lm + k + lm2 * 
