/* cdriv1.f -- translated by f2c (version 12.02.01).
...
/* DECK CDRIV1 */
/* Subroutine */ int cdriv1_(integer *n, real *t, complex *y, U_fp f, real *
	tout, integer *mstate, real *eps, complex *work, integer *lenw, 
	integer *ierflg)
{
    /* Initialized data */
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "CDRIV1", ch__1, ierflg, &c__1, (ftnlen)6, (ftnlen)
		6, (ftnlen)79);
	*mstate = i_sign(&c__7, mstate);
	return 0;
    } else if (abs(*mstate) == 7) {
	*ierflg = 999;
	xermsg_("SLATEC", "CDRIV1", "Illegal input.  The magnitude of MSTATE"
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "CDRIV1", ch__2, ierflg, &c__1, (ftnlen)6, (ftnlen)
		6, (ftnlen)93);
	*mstate = i_sign(&c__7, mstate);
	return 0;
    }
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "CDRIV1", ch__3, ierflg, &c__1, (ftnlen)6, (ftnlen)
		6, (ftnlen)95);
	*mstate = i_sign(&c__7, mstate);
	return 0;
    }
...
/* L20: */
	    i__3 = i__ - 1;
	    i__4 = i__ + lenwcm;
	    iwork[i__3] = work[i__4].r;
	}
    }
    cdriv3_(n, t, &y[1], (U_fp)f, &nstate, tout, &ntask, &c__0, eps, ewtcom, &
...
/* L40: */
	i__4 = i__ + lenwcm;
	i__2 = i__ - 1;
	work[i__4].r = (real) iwork[i__2], work[i__4].i = 0.f;
    }
    if (nstate <= 4) {
