/* la05cs.f -- translated by f2c (version 12.02.01).
...
/* DECK LA05CS */
/* Subroutine */ int la05cs_(real *a, integer *ind, integer *ia, integer *n, 
	integer *ip, integer *iw, real *w, real *g, real *u, integer *mm)
{
    ...
    static char xern1[8];
    extern /* Subroutine */ int la05es_(real *, integer *, integer *, integer 
	    *, integer *, integer *, logical *), xermsg_(char *, char *, char 
	    *, integer *, integer *, ftnlen, ftnlen, ftnlen), xsetun_(integer 
	    *);
...
/* INSERT NEW COLUMN */
	...
	if ((r__1 = w[i__], dabs(r__1)) <= la05ds_1.small) {
	    goto L100;
	}
	++la05ds_1.lenu;
	last = ii;
...
/* COMPRESS COLUMN FILE IF NECESSARY. */
	if (la05ds_1.ncp >= mcp || la05ds_1.lenl + la05ds_1.lenu >= *ia) {
	    goto L610;
	}
...
L40:
	...
	nz = iw[jm + (iw_dim1 << 1)];
	if (nz == 0) {
	    ip[jm + (ip_dim1 << 1)] = la05ds_1.lcol;
	}
	iw[jm + (iw_dim1 << 1)] = nz + 1;
	ind[la05ds_1.lcol + ind_dim1] = i__;
	nz = iw[i__ + iw_dim1];
	kpl = ip[i__ + ip_dim1] + nz;
	if (kpl > la05ds_1.lrow) {
	    goto L50;
	}
...
L50:
	if (la05ds_1.lenl + la05ds_1.lrow + nz < *ia) {
	    goto L60;
	}
	if (la05ds_1.ncp >= mcp || la05ds_1.lenl + la05ds_1.lenu + nz >= *ia) 
		{
	    goto L610;
	}
...
L60:
	kp = ip[i__ + ip_dim1];
	ip[i__ + ip_dim1] = la05ds_1.lrow + 1;
	if (nz == 0) {
	    goto L80;
	}
	kpl = kp + nz - 1;
	...
	for (k = kp; k <= i__2; ++k) {
	    ++la05ds_1.lrow;
	    a[la05ds_1.lrow] = a[k];
	    ind[la05ds_1.lrow + (ind_dim1 << 1)] = ind[k + (ind_dim1 << 1)];
	    ind[k + (ind_dim1 << 1)] = 0;
/* L70: */
	}
L80:
...
L300:
	...
	if ((r__1 = a[kp], dabs(r__1)) < *u * dabs(am)) {
	    goto L310;
	}
	if (dabs(am) < *u * (r__1 = a[kp], dabs(r__1))) {
	    goto L340;
	}
...
/* COMPRESS ROW FILE UNLESS IT IS CERTAIN THAT THERE IS ROOM FOR NEW ROW. */
	...
	if (la05ds_1.ncp >= mcp || la05ds_1.lenu + iw[ir + iw_dim1] + iw[ipp 
		+ iw_dim1] + la05ds_1.lenl > *ia) {
	    goto L610;
	}
	la05es_(&a[1], &ind[(ind_dim1 << 1) + 1], &ip[ip_offset], n, &iw[
		iw_offset], ia, &c_true);
...
/* COMPRESS COLUMN FILE IF THERE IS NOT ROOM FOR NEW ENTRY. */
	    if (la05ds_1.ncp >= mcp || la05ds_1.lenu + la05ds_1.lenl + nz + 1 
		    >= *ia) {
		goto L610;
	    }
	    la05es_(&a[1], &ind[ind_offset], &ip[(ip_dim1 << 1) + 1], n, &iw[(
...
/* Writing concatenation */
	...
	xermsg_("SLATEC", "LA05CS", ch__1, &c_n6, &c__1, (ftnlen)6, (ftnlen)6,
		 (ftnlen)62);
    }
    *g = -6.f;
    return 0;
...
L610:
    if (la05ds_1.lp > 0) {
	xermsg_("SLATEC", "LA05CS", "LENGTHS OF ARRAYS A(*) AND IND(*,2) ARE"
...
L620:
    if (la05ds_1.lp > 0) {
	xermsg_("SLATEC", "LA05CS", "EARLIER ENTRY GAVE ERROR RETURN.", &c_n8,
