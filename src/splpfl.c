/* splpfl.f -- translated by f2c (version 12.02.01).
...
/* DECK SPLPFL */
/* Subroutine */ int splpfl_(integer *mrelas, integer *nvars__, integer *
	ienter, integer *ileave, integer *ibasis, integer *ind, integer *ibb, 
	real *theta, real *dirnrm, real *rprnrm, real *csc, real *ww, real *
	bl, real *bu, real *erp, real *rprim, real *primal, logical *finite, 
	logical *zerolv)
{
...
/*     THE STEP LENGTH. */
L20009:
    if (! ((r__1 = ww[i__], dabs(r__1)) <= *dirnrm * erp[i__])) {
	goto L20012;
    }
    goto L20005;
L20012:
...
/*     IF RPRIM(I) IS ESSENTIALLY ZERO, SET RATIO TO ZERO AND EXIT LOOP. */
    if (! ((r__1 = rprim[i__], dabs(r__1)) <= *rprnrm * erp[i__])) {
	goto L20018;
    }
    *theta = zero;
    *ileave = i__;
...
L20037:
    ...
    *zerolv = *zerolv && (r__1 = *theta * ww[i__], dabs(r__1)) <= erp[i__] * *
	    rprnrm;
    if (*zerolv) {
	goto L20040;
    }
