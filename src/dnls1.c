/* dnls1.f -- translated by f2c (version 12.02.01).
	...
	doublereal *wa3, doublereal *wa4)
{
    ...
    extern /* Subroutine */ int dqrfac_(integer *, integer *, doublereal *, 
	    ...
	    doublereal *, doublereal *);
    static doublereal actred, dirder, epsmch, prered;
...
/*           EVALUATE FUNCTION AT INCREMENTED VALUE AND PUT IN WA4(*). */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    modech = 2;
	    dckder_(&c__1, n, &x[1], &fvec[i__], &fjac[i__ + fjac_dim1], 
...
/* Writing concatenation */
		...
		xermsg_("SLATEC", "DNLS1", ch__1, &c__7, &c__0, (ftnlen)6, (
			ftnlen)5, (ftnlen)84);
	    }
/* L350: */
	}
...
/*        COMPUTE THE QR FACTORIZATION OF THE JACOBIAN. */
    ...
    dqrfac_(m, n, &fjac[fjac_offset], ldfjac, &c_true, &ipvt[1], n, &wa1[1], &
	    wa2[1], &wa3[1]);
...
L495:
	modech = 2;
	dckder_(&c__1, n, &x[1], &fvec[i__], &wa3[1], &c__1, &wa1[1], &wa4[
		i__], &modech, &err);
	if (err < chklim) {
	    s_wsfi(&io___26);
	    do_fio(&c__1, (char *)&i__, (ftnlen)sizeof(integer));
...
/* L510: */
    ...
    dqrfac_(n, n, &fjac[fjac_offset], ldfjac, &c_true, &ipvt[1], n, &wa1[1], &
	    wa2[1], &wa3[1]);
    i__1 = *n;
    for (j = 1; j <= i__1; ++j) {
	if (fjac[j + j * fjac_dim1] == zero) {
