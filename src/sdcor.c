/* sdcor.f -- translated by f2c (version 12.02.01).
...
/* DECK SDCOR */
/* Subroutine */ int sdcor_(real *dfdy, real *el, S_fp fa, real *h__, integer 
	*ierror, integer *impl, integer *ipvt, integer *matdim, integer *
	miter, integer *ml, integer *mu, integer *n, integer *nde, integer *
	nq, real *t, S_fp users, real *y, real *yh, real *ywt, logical *
	evalfa, real *save1, real *save2, real *a, real *d__, integer *jstate)
{
...
/* Computing MAX */
		...
		save1[i__] = (*h__ * save2[i__] - yh[i__ + (yh_dim1 << 1)] - 
			save1[i__]) / dmax(r__2,r__3);
/* L102: */
	    }
	}
	*d__ = snrm2_(n, &save1[1], &c__1) / sqrt((real) (*n));
	i__1 = *n;
	for (i__ = 1; i__ <= i__1; ++i__) {
/* L105: */
	    save1[i__] = *h__ * save2[i__] - yh[i__ + (yh_dim1 << 1)];
...
/* L170: */
	...
	sgesl_(&dfdy[dfdy_offset], matdim, n, &ipvt[1], &save2[1], &c__0);
	if (*ierror == 1 || *ierror == 5) {
	    i__1 = *n;
	    for (i__ = 1; i__ <= i__1; ++i__) {
		save1[i__] += save2[i__];
...
/* Computing MAX */
		r__2 = (r__1 = y[i__], dabs(r__1)), r__3 = ywt[i__];
		save2[i__] /= dmax(r__2,r__3);
	    }
	}
	*d__ = snrm2_(n, &save2[1], &c__1) / sqrt((real) (*n));
...
/* L290: */
	...
	sgbsl_(&dfdy[dfdy_offset], matdim, n, ml, mu, &ipvt[1], &save2[1], &
		c__0);
	if (*ierror == 1 || *ierror == 5) {
	    i__3 = *n;
