/* xpmup.f -- translated by f2c (version 12.02.01).
...
/* DECK XPMUP */
/* Subroutine */ int xpmup_(real *nu1, real *nu2, integer *mu1, integer *mu2, 
...
/* ***FIRST EXECUTABLE STATEMENT  XPMUP */
    ...
    *ierror = 0;
    ...
    mu = *mu1;
    dmu = (real) mu;
    n = (integer) (*nu2 - *nu1 + .1f) + (*mu2 - *mu1) + 1;
    j = 1;
    if (r_mod(&nu, &c_b2) != 0.f) {
	goto L210;
    }
L200:
    if (dmu < nu + 1.f) {
...
L225:
	...
	goto L240;
L230:
	prod = prod * (-dmu - nu - 1.f) / (dmu - nu - 1.f);
	xadj_(&prod, &iprod, ierror);
	if (*ierror != 0) {
	    return 0;
	}
