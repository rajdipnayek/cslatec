/* dbsk0e.f -- translated by f2c (version 12.02.01).
...
doublereal dbsk0e_(doublereal *x)
{
    ...
    static doublereal ak0cs[38] = { -.07643947903327941424082978270088,
    ...
    static doublereal ak02cs[33] = { -.01201869826307592239839346212452,
    ...
    static real eta;
    static integer ntk0;
    static doublereal xsml;
    static integer ntak0, ntak02;
    extern doublereal d1mach_(integer *), dbesi0_(doublereal *), dcsevl_(
...
/* ***FIRST EXECUTABLE STATEMENT  DBSK0E */
	...
	ntk0 = initds_(bk0cs, &c__16, &eta);
	ntak0 = initds_(ak0cs, &c__38, &eta);
	ntak02 = initds_(ak02cs, &c__33, &eta);
	xsml = sqrt(d1mach_(&c__3) * 4.);
    }
    first = FALSE_;

    if (*x <= 0.) {
	xermsg_("SLATEC", "DBSK0E", "X IS ZERO OR NEGATIVE", &c__2, &c__2, (
    ...
    d__1 = y * .5 - 1.;
    ret_val = exp(*x) * (-log(*x * .5) * dbesi0_(x) - .25 + dcsevl_(&d__1, 
	    bk0cs, &ntk0));
    return ret_val;
...
L20:
    if (*x <= 8.) {
	d__1 = (16. / *x - 5.) / 3.;
	ret_val = (dcsevl_(&d__1, ak0cs, &ntak0) + 1.25) / sqrt(*x);
    }
    if (*x > 8.) {
	d__1 = 16. / *x - 1.;
	ret_val = (dcsevl_(&d__1, ak02cs, &ntak02) + 1.25) / sqrt(*x);
    }
