/* bint4.f -- translated by f2c (version 12.02.01).
...
/* DECK BINT4 */
/* Subroutine */ int bint4_(real *x, real *y, integer *ndata, integer *ibcl, 
	integer *ibcr, real *fbcl, real *fbcr, integer *kntopt, real *t, real 
	*bcoef, integer *n, integer *k, real *w)
{
    ...
    static real tol;
    static integer iwp;
    static real txn, work[15];
    extern /* Subroutine */ int bnfac_(real *, integer *, integer *, integer *
	    , integer *, integer *);
    static integer iflag, ileft;
    extern /* Subroutine */ int bspvd_(real *, integer *, integer *, real *, 
	    integer *, integer *, real *, real *), bnslv_(real *, integer *, 
	    integer *, integer *, integer *, real *);
    static real wdtol, vnikx[16]	/* was [4][4] */;
...
/*     RIGHT LIMITS */
    it = *ibcl + 1;
    bspvd_(&t[1], k, &it, &x[1], k, &c__4, vnikx, work);
    iw = 0;
    if (dabs(vnikx[2]) < tol) {
	iw = 1;
    }
...
/*     SET UP INTERPOLATION EQUATIONS FOR POINTS I=2 TO I=NDATA-1 */
	...
	++ileft;
	bspvd_(&t[1], k, &c__1, &x[i__], &ileft, &c__4, vnikx, work);
	for (j = 1; j <= 3; ++j) {
...
L170:
    it = *ibcr + 1;
    bspvd_(&t[1], k, &it, &x[*ndata], &ileft, &c__4, vnikx, work);
    jw = 0;
    if (dabs(vnikx[1]) < tol) {
	jw = 1;
    }
...
/*     SOLVE SYSTEM OF EQUATIONS */
    ...
    nwrow = 5;
    iwp = iw + 1;
    bnfac_(&w[iwp + 5], &nwrow, n, &ilb, &iub, &iflag);
    if (iflag == 2) {
	goto L190;
    }
    bnslv_(&w[iwp + 5], &nwrow, n, &ilb, &iub, &bcoef[1]);
    return 0;


L190:
    xermsg_("SLATEC", "BINT4", "THE SYSTEM OF EQUATIONS IS SINGULAR", &c__2, &
	    c__1, (ftnlen)6, (ftnlen)5, (ftnlen)35);
    return 0;
L200:
    xermsg_("SLATEC", "BINT4", "NDATA IS LESS THAN 2", &c__2, &c__1, (ftnlen)
	    6, (ftnlen)5, (ftnlen)20);
    return 0;
L210:
    xermsg_("SLATEC", "BINT4", "X VALUES ARE NOT DISTINCT OR NOT ORDERED", &
	    c__2, &c__1, (ftnlen)6, (ftnlen)5, (ftnlen)40);
    return 0;
L220:
    xermsg_("SLATEC", "BINT4", "IBCL IS NOT 1 OR 2", &c__2, &c__1, (ftnlen)6, 
	    (ftnlen)5, (ftnlen)18);
    return 0;
L230:
    xermsg_("SLATEC", "BINT4", "IBCR IS NOT 1 OR 2", &c__2, &c__1, (ftnlen)6, 
	    (ftnlen)5, (ftnlen)18);
    return 0;
L240:
    xermsg_("SLATEC", "BINT4", "KNTOPT IS NOT 1, 2, OR 3", &c__2, &c__1, (
	    ftnlen)6, (ftnlen)5, (ftnlen)24);
    return 0;
L250:
    xermsg_("SLATEC", "BINT4", "KNOT INPUT THROUGH W ARRAY IS NOT ORDERED PR"
	    "OPERLY", &c__2, &c__1, (ftnlen)6, (ftnlen)5, (ftnlen)50);
    return 0;
} /* bint4_ */
